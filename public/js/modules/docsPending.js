    ...
    docs.forEach(doc => {
        ...
        tr.innerHTML = `
            ...
            <input type="hidden" class="doc-id" name="action_id" value="${escapeHtml(doc.action_id)}">
                ...
                <button class="btn btn-sm btn-danger defer" aria-label="Deferred document">
                    <i class="fas fa-clock"></i>
                </button>
                <button class="btn btn-sm btn-success release" aria-label="Release document">
...
export async function releaseDocument(actionId, remarks, releaseCopy, releaseTO, releaseLogbookinfo) {
    if (!isOnline()) {
        throw new Error('Cannot release document while offline');
    }
    ...
    try {
        ...
        console.log('Sending release request:', {
            url: `/api/documents/routes/${actionId}/release`,
            actionId,
            remarks,
            releaseCopy,
            releaseTO,
        ...
        const response = await fetch(`/api/documents/routes/${actionId}/release`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
            ...
            body: JSON.stringify({
                ...
                doc_copy: releaseCopy || 1,
                release_to: releaseTO || null,
                logbook_page: releaseLogbookinfo || null 
            })
        });
        ...
        if (!response.ok) {
            throw new Error(responseData.message || responseData.error || 'Failed to release document');
        }

        return responseData;

    } catch (error) {
        console.error('Error in releaseDocument:', error);
        throw new Error(`Error releasing document: ${error.message}`);
    }
}
